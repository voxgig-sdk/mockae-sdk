...
(0, node_test_1.describe)('CouponDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-coupon', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-coupon', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.MOCKAE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.MockaeSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['MOCKAE_TEST_COUPON_ENTID'];
    ...
    const client = new __1.MockaeSDK({
        base: 'http://localhost:8080',
