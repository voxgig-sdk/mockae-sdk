...
//     (\`new MockaeSDK(...)\` / \`MockaeSDK.test(...)\` become
//     \`MockaeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cart":{"test01":{"id":"test01"}},"coupon":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}},"status":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MockaeSDK(...)\` and any
// \`MockaeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MockaeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MockaeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
