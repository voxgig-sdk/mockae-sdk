...
class MockaeNetsimFeature extends MockaeBaseFeature
{
    ...
    private function _simulate(MockaeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(MockaeContext $ctx, array $applied): void
    {
        $client = $this->client;
