...
class MockaeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MockaeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MockaeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MockaeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MockaeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MockaeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MockaeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MockaeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MockaeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MockaeError($op . "_allow",
                "MockaeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MockaeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MockaeError("graphql_error",
                "MockaeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
