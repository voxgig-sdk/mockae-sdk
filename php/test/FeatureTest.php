...
class FtCtrl extends MockaeControl
{
...
class FtClient
{
    ...
    private MockaeUtility $utility;

    public function __construct(MockaeUtility $utility, array $options)
    {
    ...
    public function get_utility(): MockaeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MockaeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MockaeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MockaeConfig::make_feature($name);
    ...
    private static function buildUrl(MockaeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MockaeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MockaeResponse($fetched);
        }
    ...
    private function fail(MockaeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MockaeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
