...
{
    private static function utility(): MockaeUtility
    {
        return new MockaeUtility();
    }
    ...
    private static function ctx(array $over = []): MockaeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MockaeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MockaeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MockaeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MockaeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MockaeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MockaeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MockaeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MockaeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MockaeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MockaeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MockaeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MockaeResponse([]),
            'result' => new MockaeResult([]),
        ]);
        [, $err] = MockaeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MockaeSpec(['step' => 's']),
            'response' => new MockaeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MockaeResult([]),
        ]);
        [, $err] = MockaeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MockaeSpec(['step' => 's']),
            'response' => new MockaeResponse(self::resp(200, ['v' => 1])),
            'result' => new MockaeResult([]),
        ]);
        [, $err] = MockaeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MockaeSpec(['step' => 's']),
            'response' => new MockaeResponse(self::resp(200, ['v' => 2])),
            'result' => new MockaeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MockaeMakeResponse::call($ctx);
    ...
    {
        $preset = new MockaeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MockaeSpec([]),
            'response' => new MockaeResponse([]),
            'result' => new MockaeResult([]),
        ...
        [$got, $err] = MockaeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MockaeResult([])]);
        [, $err] = MockaeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MockaeSpec([]), 'result' => null]);
        [, $err] = MockaeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MockaeSpec(['step' => 's']),
            'result' => new MockaeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MockaeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MockaeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MockaeSpec(['step' => 's']),
            'result' => new MockaeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MockaeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MockaeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MockaeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MockaeSpec([]),
            'result' => new MockaeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MockaeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MockaeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MockaeError('boom', 'boom');
        $utility->fetcher = function (MockaeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MockaeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MockaeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MockaeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MockaeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MockaeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MockaeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MockaeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MockaeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MockaeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MockaeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MockaeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MockaeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MockaeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MockaeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MockaeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MockaeResult(['ok' => false])]);
        $this->expectException(MockaeError::class);
        MockaeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MockaeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MockaeMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MockaeBaseFeature();
        $b = new MockaeBaseFeature();
        MockaeFeatureAdd::call($ctx, $a);
        MockaeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MockaeBaseFeature
    {
        $f = new MockaeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MockaeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MockaeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MockaeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MockaePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MockaeResponse(['headers' => null]),
            'result' => new MockaeResult([]),
        ]);
        MockaeResultHeaders::call($ctx);
