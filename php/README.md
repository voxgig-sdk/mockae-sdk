...
The PHP SDK for the Mockae API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Cart(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Mockae(null)->load(
    ["id" => "test01"], null
);
