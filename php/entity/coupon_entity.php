...
class CouponEntity
{
    ...
    {
        if ($args) {
            $this->_data = MockaeHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MockaeHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param CouponLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CouponLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Coupon|array The loaded Coupon as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MockaeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
