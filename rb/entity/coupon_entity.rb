...
class CouponEntity
  ...
  def data_set(args)
    if args
      @_data = MockaeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MockaeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Coupon.
  #
  # @param reqmatch [CouponLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Coupon.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Coupon, Hash] the loaded Coupon; raises MockaeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MockaeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Coupon>, Array] the matching Coupon items; raises MockaeError on failure
