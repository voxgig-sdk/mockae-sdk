...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MockaeOmni.make_runner(TEST_JSON_FILE, MockaeSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MockaeOmni.errify(err)
    end
