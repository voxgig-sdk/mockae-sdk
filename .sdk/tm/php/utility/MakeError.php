...
class MockaeMakeError
{
    public static function call(?MockaeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MockaeContext([], null);
        }
        $op = $ctx->op ?? new MockaeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MockaeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MockaeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MockaeError('', $msg, $ctx);
