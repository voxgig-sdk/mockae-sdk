...
//    only - a typed MockaeContext there would make every ctx
...
final class MockaeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MockaeContext($clean, null);
    ...
    public static function err_from_map($m): ?MockaeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MockaeError(is_string($code) ? $code : '', $msg);
    }
