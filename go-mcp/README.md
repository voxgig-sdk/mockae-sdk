...
MCP server exposing the Mockae SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `mockae_list` | `entity`, optional `query` map | First page of records as JSON |
| `mockae_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `mockae_list` and
`mockae_load` tools become available in new sessions.
