...
[MCP](https://modelcontextprotocol.io) server exposing the Mockae SDK as
two agent tools — `mockae_list` and `mockae_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mockae-mcp)
make build

# 2. Provide credentials via the environment
export MOCKAE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mockae \
  -- /absolute/path/to/mockae-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mockae-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mockae_list: first page of records
{ "entity": "cart" }
{ "entity": "cart", "query": { } }

// mockae_load: one record by id
{ "entity": "cart", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mockae-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mockae \
     -- "$PWD"/dist/*/mockae-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mockae_list` and `mockae_load` tools now appear
   in new sessions. Ask the agent to *"list cart using mockae"*
   and it calls `mockae_list` with `{"entity":"cart"}`.
...
```sh
export MOCKAE_APIKEY=sk_live_xxx            # API key
export MOCKAE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `mockae_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mockae_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOCKAE_APIKEY` | API key sent with every request. |
| `MOCKAE_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

cart | coupon | product | status | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./mockae-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mockae_load","arguments":{"entity":"cart","query":{"id":1}}}}'
```
