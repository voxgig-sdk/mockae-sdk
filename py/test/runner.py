# Mockae SDK test runner

from __future__ import annotations
import os
import json

from mockae_sdk.utility.voxgig_struct import voxgig_struct as vs


class MockaeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MockaeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MockaeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MockaeTestRunner.getenv("MOCKAE_TEST_LIVE")
        override = MockaeTestRunner.getenv("MOCKAE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MockaeTestRunner.getenv(key)
        ...
        explain = MockaeTestRunner.getenv("MOCKAE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MOCKAE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MockaeTestRunner._test_control is not None:
            return MockaeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MockaeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MockaeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MockaeTestRunner.load_test_control()
...
def load_env_local():
    MockaeTestRunner.load_env_local()


def env_override(m):
    return MockaeTestRunner.env_override(m)


def entity_data(v):
    return MockaeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MockaeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MockaeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MockaeTestRunner.load_test_control()


def live_client_options():
    return MockaeTestRunner.live_client_options()


def live_delay_ms():
    return MockaeTestRunner.live_delay_ms()
