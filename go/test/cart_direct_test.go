...
func TestCartDirect(t *testing.T) {
	...
	t.Run("direct-load-cart", func(t *testing.T) {
		setup := cartDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-cart", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func cartDirectSetup(mockres any) *cartDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MOCKAE_TEST_CART_ENTID"]; ok {
