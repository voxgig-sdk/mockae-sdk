...
import (
	...
	omni "github.com/voxgig-sdk/mockae-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MockaeError); is {
				out := map[string]any{
					"name":    "MockaeError",
...
func omniCtx(arg any, client *sdk.MockaeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MockaeSDK); is {
			client = live
