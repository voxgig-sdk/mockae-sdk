...
func (sdk *MockaeSDK) OptionsMap() map[string]any {
...
func (sdk *MockaeSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MockaeSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MockaeSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MockaeSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MockaeSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MockaeSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MockaeSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MockaeSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MockaeSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MockaeSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
